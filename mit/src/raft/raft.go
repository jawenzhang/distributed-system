...
func (rf *Raft) RequestVote(args RequestVoteArgs, reply *RequestVoteReply) {
	...
	if args.Term < currentTerm {
		...
		reply.VoteGranted = false
	} else if lastLog := rf.log[len(rf.log)-1]; lastLog.Term > args.LastLogTerm || (lastLog.Term == args.LastLogTerm && lastLog.Index > args.LastLogIndex ){
...
func (rf *Raft) AppendEnties(args AppendEntiesArgs, reply *AppendEntiesReply) {
	...
	if (len(rf.log)) < args.PrevLogIndex || rf.log[args.PrevLogIndex].Term != args.PrevLogTerm {
		...
		// 更新commitIndex,过早更新会导致发送过来的log还没有拷贝到本地的rf.log中
